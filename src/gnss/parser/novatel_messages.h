...
struct BDS_Ephemeris 
{
  ...
  double crc;  // Amplitude of cosine harmonic correction term to the orbit radius(meters)
  double crs;  // Amplitude of sine harmonic correction term to the orbit radius(meters)
...
struct GLO_Ephemeris 
{
  ...
  double vel_x;  // X coordinate for satellite velocity at reference time(PZ-90.02), in meters/s
  double vel_y;  // Y coordinate for satellite velocity at reference time(PZ-90.02), in meters/s
  double vel_z;  // Z coordinate for satellite velocity at reference time(PZ-90.02), in meters/s
  ...
  double delta_tau_n;  // Time difference between navigation RF signal transmitted in L2 sub-band and navigation RF signal transmitted in L1 sub-band by nth satellite , in seconds
...
struct GPS_Ephemeris 
{
  ...
  double ecc;        // Eccentricity. dimensionless-quantity defined for a conic section where e=0 is a circle, e=1 is a parabola 0<e<1 os an ellipse and e>1 is a hyperbola
  double omega;      // Argument of perigee, radians -measurement along the orbital path from the ascending node to the point where the SV os closest to the earth, in the direction of the SV's motion
