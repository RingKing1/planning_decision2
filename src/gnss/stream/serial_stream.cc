...
void SerialStream::open(void)
{
  ...
  if (fd == -1)
  {
    ...
    case EMFILE:
    default:
      std::cout << "Open device " << device_name_
...
void SerialStream::check_remove()
{
  ...
  if (nsent < 0)
  {
    std::cout << "Serial stream detect write failed, error: " << strerror(errno) << "\n";
    switch (errno)
    {
    case EBADF:
    case EIO:
      status_ = Stream::Status::DISCONNECTED;
      std::cout << "Device " << device_name_ << " removed.\n";
      Disconnect();
      break;
    }
  }
...
size_t SerialStream::read(uint8_t *buffer, size_t max_length)
{
  if (!is_open_)
  {
    ...
    std::cout << "Connect " << device_name_ << " success.\n";
  }
  ...
  while (max_length > 0)
  {
    ...
    if (bytes_current_read < 0)
    {
      ...
      case EBADF:
      case EIO:
        std::cout << "Serial stream read data failed, error: "
                  << strerror(errno) << "\n"; 
        Disconnect();
        if (Connect())
        {
          std::cout << "Reconnect " << device_name_ << " success.\n";
          bytes_current_read = 0;
          break; // has recoverable
        }

      default:
        std::cout << "Serial stream read data failed, error: " << strerror(errno)
                  << ", errno: " << errno << "\n";
        status_ = Stream::Status::ERROR;
        errno_ = errno;
        return bytes_read;
...
size_t SerialStream::write(const uint8_t *data, size_t length)
{
  ...
  while ((length > 0) && (delay_times < 5))
  {
    ...
    if (nsent < 0)
    {
      std::cout << "Serial stream write data failed, error: " << strerror(errno) << "\n";
      switch (errno)
      {
      case EAGAIN:
      case EINVAL:
