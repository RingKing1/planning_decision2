...
/**/
	...
	void bezier_normal(const Eigen::MatrixXd& points, const double& allSize, Eigen::MatrixXd &trajs){
		/*bezier_normal:给定离散点序列产生规范贝塞尔曲线
		*Input: 
		*points  :参考点序列
		*allSize :生成轨迹点个数
		*Return :
		...
		*/
		Eigen::Vector2d p_t;
		int n;
		Eigen::VectorXd t_len;// linspace(low,high,size)'线性分布的数组
		t_len.setLinSpaced(allSize, 0, 1);
		trajs.resize(2,  t_len.size());
		for(int t = 0; t < t_len.size(); t++){
			if (n == 1){
				p_t =  points.col(0);
			}
			p_t << 0, 0;
			n = points.cols()- 1;
			for(int i = 0; i <= n; i++){
				double C_n_i = std::tgamma(n + 1) / (std::tgamma(i + 1) * std::tgamma(n - i + 1));
				p_t =  p_t.array() + C_n_i * pow(1 - t_len(t), n - i) *  pow(t_len(t), i) * points.col(i).array();
			}
			trajs.col(t)  = p_t;
		}   
	}
