...
double DigitalFilter::Filter(const double x_insert) {
  if (denominators_.empty() || numerators_.empty()) {
    std::cout << "Empty denominators or numerators"<<endl;
    return 0.0;
  }
  x_values_.pop_back();  //前面是最新的，后面是最老的。
  x_values_.push_front(x_insert);
  const double xside =
      Compute(x_values_, numerators_, 0, numerators_.size() - 1);

    //std::cout<<"shanchuqian"<<y_values_[0]<<" "<<y_values_[1]<<" "<<y_values_[2]<<endl;
  y_values_.pop_back();
    //std::cout<<y_values_[0]<<" "<<y_values_[1]<<" "<<y_values_[2]<<endl;
...
double DigitalFilter::Compute(const std::deque<double> &values,
  ...
  double sum = 0.0;
  size_t i = coeff_start;
  for (size_t j = 0; j < values.size(); j++)
  {
    sum +=  values[j] * coefficients[i];
    ++i;
  }
