...
OSQPData *MpcOsqp::Data() {
  ...
  if (data == nullptr) {
    ...
    data->P =
        csc_to_triu(csc_matrix(kernel_dim, kernel_dim, P_data.size(), CopyData(P_data),
    ...
    // data->A =
    //     csc_to_triu(csc_matrix(num_affine_constraint,
...
//   }
  ...
  c_int status = osqp_work->info->status_val;
  // osqp_solve(osqp_work);
  ...
  for (size_t i = 0; i < control_dim_; ++i) {
    control_cmd->at(i) = osqp_work->solution->x[i + first_control];
