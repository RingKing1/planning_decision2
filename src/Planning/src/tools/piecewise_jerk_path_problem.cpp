...
void PiecewiseJerkPathProblem::CalculateKernel(std::vector<c_float>* P_data,
  ...
  const int num_of_nonzeros = num_of_variables + (n - 1);//非零元素的数目
  //二维数组 
...
//设置权重 
  ...
  // 转换成csc_matrix的形式
  int ind_p = 0;
  for (int i = 0; i < num_of_variables; ++i) {//3n
...
void PiecewiseJerkPathProblem::CalculateOffset(std::vector<c_float>* q){
  ...
  // std::cout<<"has_x_ref_:"<<has_x_ref_<<std::endl;
  // std::cout<<"weight_x_ref_vec_:"<<weight_x_ref_vec_.size()<<std::endl;
  // std::cout<<"x_ref_:"<<x_ref_.size()<<std::endl;
  // std::cout<<"scale_factor_:"<<scale_factor_.size()<<std::endl;
  if (has_x_ref_) {
  ...
  // std::cout<<"has_end_state_ref_:"<<has_end_state_ref_<<std::endl;
