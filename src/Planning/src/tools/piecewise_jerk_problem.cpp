...
OSQPData* PiecewiseJerkProblem::FormulateProblem() {
  ...
  // 目标函数Hessian矩阵  目标函数中H矩阵的计算 
  CalculateKernel(&P_data, &P_indices, &P_indptr);
  // std::cout<<"P_data: "<<P_data.size()<<std::endl;
  // std::cout<<"P_indices: "<<P_indices.size()<<std::endl;
  ...
  // std::cout<<"A_data: "<<A_data.size()<<std::endl;
  // std::cout<<"A_indices: "<<A_indices.size()<<std::endl;
  // std::cout<<"A_indptr: "<<A_indptr.size()<<std::endl;
  // std::cout<<"lower_bounds: "<<lower_bounds.size()<<std::endl;
  // std::cout<<"upper_bounds: "<<upper_bounds.size()<<std::endl;
  // for (size_t i = 0; i < upper_bounds.size(); ++i)
  // {
  //   std::cout<<"i: "<<i<<" "<<"lower_bounds: "<<lower_bounds[i]<<" ";
  //   std::cout<<"upper_bounds: "<<upper_bounds[i]<<std::endl;
  // }
  // 0.5xHx + gx中的g矩阵计算
...
void PiecewiseJerkProblem::CalculateAffineConstraint(
  ...
  const int n = static_cast<int>(num_of_knots_);//static_cast<int> 强制转换为 int
  ...
  std::vector<std::vector<std::pair<c_int, c_float>>> variables(num_of_variables); //列 行 

  int constraint_index = 0;

  // set x, x', x'' bounds 设置边界 
  ...
  for (int i = 0; i + 1 < n; ++i) {
    variables[n + i    ].emplace_back(constraint_index, -1.0 * scale_factor_[2]); //l'
    variables[n + i + 1].emplace_back(constraint_index,  1.0 * scale_factor_[2]);//l(i+1) '
  ...
  int ind_p = 0;
  for (int i = 0; i < num_of_variables; ++i) {//3n //lie
    A_indptr->push_back(ind_p);
    for (const auto& variable_nz : variables[i]) {
      // coefficient hang 
      A_data->push_back(variable_nz.second);

      // constraint index zhi
